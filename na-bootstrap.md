...
for any existing entities whose `counties` field includes the target county — these are
entities discovered in a prior county run that are known to span into this county. Run the
queries in `processing/na_cross_county_resolution.md` §5 to retrieve them. The baseline
document must include a "Known Multi-County Entities" section listing all MC entities and
held entities from other counties that reference the target county. This list informs
discovery — when you encounter one of these entities during a tier, use `KNOWN_MC:{id}`
rather than `CROSS_COUNTY_CANDIDATE` in `identity_notes_raw`.
...
**Entity Sequence Numbering Gaps (IMP-117):** Entity sequence numbers (e.g., `OH-MC-SN-0001`, `OH-MC-T-0109`) may have gaps across county runs. Gaps do **not** indicate data loss or a broken sequence. They arise from:
1. Provisional IDs assigned during a Scenario A cross-county hold that are later superseded when the partner county runs and the entity rolls into an existing MC record.
2. Entities held during cross-county resolution that are ultimately merged into a pre-existing canonical record rather than receiving a new ID.
3. Sequence numbers reserved during a county run but withdrawn during QA before upsert.

Operators must not infer missing entities from sequence gaps. The `resolution_provenance` table records the full history of ID assignments for any entity whose sequence appears non-contiguous.
