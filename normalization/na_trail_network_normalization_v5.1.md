...
# TRAIL NETWORK NORMALIZATION CONTRACT v5.1
...
The Trail Network Normalization Contract v5.1 defines:
...
- Trail Network Vocabulary Module v5.x (Network Type, Status)
...
- A **Normalized Trail Network Object v5.1** conforming to the Trail
...
13. Normalize Identity Notes
14. Normalize Notes
15. Normalize URL
16. Normalize Maps (URL list)
17. Run integrity anchor deduplication check (via Normalization
    Engine)
18. Validate against Trail Network Schema v5.x
19. Emit Normalized Trail Network + provenance

If any critical step fails → return error to Normalization Engine
v5.x.
...
- Must not be inferred from member count, geographic extent, or
  governance alone.
- If ambiguous → leave blank, log uncertainty, flag in
  identity_notes.
...
- See Trail Network Vocabulary Module v5.x for full mapping table.
...
- "Partially Open" applies when some member trails are open and
  others are not yet built.
...
- Use authoritative two-letter state abbreviations (e.g., OH, IN,
  KY).
...
- Must not contain operational or contextual notes (those go in
  Notes)
...
- Use for: operational details, gap documentation, planning status,
  funding notes, partial completion notes, contextual
  clarifications.
...
- Alternate names and history migration to Description (if
  applicable)
- Maps URL validation results (valid/invalid URLs, duplicates
  removed)
...
------------------------------------------------------------
# END OF TRAIL NETWORK NORMALIZATION CONTRACT v5.1
