...
- If the normalized Trail entity has a `parent_site_id` value:
  ...
  - If Site does not yet exist: add Trail to Held Entities with hold reason `unresolved_parent`; release and insert `trail_parents` row when the Site is upserted.
...
Live DB schema (verified 2026-04-27):

| Column | Notes |
|--------|-------|
| `record_id` | PK (auto-increment integer) |
| `entity_type` | e.g. `site`, `access_point`, `trail` |
| `name` | Entity display name |
| `hold_reason` | See values below |
...
`hold_reason` allowed values:
- `gps_missing` — Site or AP without GPS, no `gps_unresolvable` flag (GPS Gate, IMP-069)
- `parent_held` — Entity whose parent is itself in `held_entities`
- `unresolved_member_ids` — Network with member IDs not yet in graph
- `unresolved_parent` — Entity with parent not yet in graph (parent was rejected or not yet discovered)

Note: `hold_detail` carries the full normalized entity JSON or a short diagnostic string depending on context.

## 8.3 Held-Entity Child Rule (IMP-086)

### When It Fires

Immediately after the GPS Gate held list is finalized at the start of Stage 3 (Normalization),
before any individual entity normalization begins. The scan runs once per pipeline run.

### Trigger Condition

Any child entity whose parent reference points to an entity already in `held_entities` for
the current run must itself be held. The child cannot be normalized against a parent that
...
**Child Sites** — scan `parent_site_id` against the held set using the same pattern.
Any child Site whose parent Site is held is itself moved to `held_entities` with
`hold_reason = parent_held`.

### Exempt Entity Type

**Trail Networks** — `member_trail_ids` referencing held trails remain in the network
record. The network is **not** held. Log as `INFO`; the dangling member reference will
resolve when the member trail's county run completes. Do not log as `WARNING`.

### Hold Detail Format

`hold_detail` must identify the parent: `"Parent entity {parent_id} is held pending resolution"`.
This ensures the release workflow (§8.4) can locate the parent row efficiently.

### Cross-Reference

The Normalization Engine documents this rule from the caller's perspective in §1.3
(Pre- and Intra-Normalization Hold Conditions). This section is the authoritative
specification of the rule's logic and DB writes.

## 8.4 Release Workflow

On each subsequent normalization run, before Stage 3 normalization begins:

1. **GPS-missing holds** (`hold_reason = gps_missing`): Re-run GPS Acquisition (Stage 2b) for each held entity name + county. If GPS is now resolvable, remove the `held_entities` row and route the entity through Stage 3 normalization normally.
2. **Parent-held holds** (`hold_reason = parent_held`): Check if the parent entity has since been upserted into its core table. If so, remove the `held_entities` row and re-process the child through Stage 3.
3. **Unresolved member IDs** (`hold_reason = unresolved_member_ids`): Check if all referenced member entity IDs now exist in the graph. If so, remove the row and re-process.
4. **Unresolved parent** (`hold_reason = unresolved_parent`): Check if the parent entity now exists. If so, remove the row and re-process.
5. Rows held for more than 90 days with no resolution are flagged in the QA report for manual review or abandonment — delete the row after review if no longer actionable.
