...
# ACCESS POINT NORMALIZATION CONTRACT v5.3
...
**IMP-114 — AP-to-Site reclassification pathway.**

- **§10a AP-to-Site Reclassification**: New section added between §10 Missing Data
  and §11 Auditability Requirements. Defines qualifying criteria, required steps,
  and prohibitions for reclassifying an Access Point as a Site during normalization.
...
An Access Point may be reclassified as a Site during normalization when post-normalization
evidence indicates the entity has site-level identity that was not recognized at discovery time.
This pathway is intentionally narrow — most APs should remain APs.

## Qualifying Criteria (all three must be met)

1. **Acreage is populated** — `acres_raw` contains a numeric value sourced from an authoritative
   record (not inferred from map extent).
2. **Description is present** — `description_raw` contains narrative prose describing the entity
   as a place (not merely as a point of access to another entity).
3. **Governance is distinct from the parent trail or site** — `governance_raw` names an entity
   that independently manages this location as a destination, not solely as an access corridor.

## Disqualifying Conditions (any one is sufficient to block reclassification)

- Entity's identity is access-only: its entire purpose is to provide entry to a parent trail
  or site; it has no independent program, named boundary, or distinct visitor offering.
- Acreage is inferred (e.g., estimated from GIS parcel extent) rather than stated by source.
- Governance is the same managing entity as the parent, providing access under that entity's
  broader management authority.
- Description_raw describes the access function ("boat launch ramp serving the Sandusky River")
  rather than the place ("Portage River Access Area is a 3.2-acre day-use area...").

## Required Reclassification Steps

1. **Remove the entity from AP output.** Do not include it in the Access Points TSV or upsert.
2. **Create a Site record.** Infer `category` from governance and description (typically
   "Recreation Facility" or "Water Site"). Populate all Site schema fields available from
   the AP's raw fields. Carry over `gps_lat_raw`, `gps_lon_raw`, `location_raw`, `acres_raw`.
3. **Update parent references.** Any Trail or Site that listed this AP as an access point must
   be noted as having one fewer documented AP. Log the reference update.
4. **Log to normalization_provenance.**
   ```python
   {
     "entity_id": "<former AP ID>",
     "action": "ap_to_site_reclassification",
     "reclassification_reason": "<specific criteria met>",
     "new_entity_type": "Site",
     "new_entity_id": "<assigned Site ID>"
   }
   ```
5. **Flag for human review at Stage 5.5.** Reclassified entities must appear in the Human
   Review Gate checklist. Do not allow a reclassification to proceed silently to upsert.

## Prohibition

Never reclassify an AP as a Site based on map area or inferred size alone. The qualifying
criteria require explicit source-documented acreage, description, and governance evidence.
When criteria are borderline, retain the AP classification and add a note to the AP's
`identity_notes_raw` for future review.
...
------------------------------------------------------------
# END OF ACCESS POINT NORMALIZATION CONTRACT v5.3
