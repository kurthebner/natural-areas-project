...
# SITE NORMALIZATION CONTRACT v5.11
...
**IMP-116 — Features normalization Step 5 unmapped token logging.**

- **§5.18 Features**: Step 5 Unmapped Token Logging added after Step 4 Vocabulary mapping.
  Any token that survives Steps 1–3 but has no vocabulary match in Step 4 must be logged
  to `normalization_provenance` as `unmapped_token_dropped`. Accumulated unmapped tokens
  are surfaced in Stage 5 TSV Integrity Check output as vocabulary expansion candidates.
  This makes synonym gaps visible without blocking the pipeline.
...
- **IMP-067 — GPS county check (§5.17b)**: After §5.17a GPS Gate, cross-check the
  entity's `counties` field against the county derived from a point-in-polygon lookup
  on its GPS coordinates (TIGER COUSUB COUNTYFP, HIGH/MED confidence GPS only).
  On mismatch: route to `manual_review_queue` with flag='county_mismatch'; entity
  continues to output with existing counties value. ID correction permitted at manual
  review stage (before cross-county run) via cascading update to all relationship tables.

- **IMP-069 — GPS gate before output**: Added §5.17a GPS Gate. No site may proceed to TSV Output
...
**This check runs after §5.17a (GPS Gate), only when GPS is confirmed (gps_confidence HIGH or MED).**

Cross-check the entity's `counties` field against the county derived from a point-in-polygon
lookup on its GPS coordinates, using the TIGER COUSUB COUNTYFP data already loaded by
`na_township_lookup.py`.

```python
from utilities.na_township_lookup import OhioTownshipLookup
lookup = OhioTownshipLookup()

if (site.gps_confidence in ('HIGH', 'MED')
        and site.gps_lat is not None and site.gps_lon is not None):
    gps_county = lookup.get_county_name(site.gps_lat, site.gps_lon)  # None if outside Ohio
    documented = [c.strip() for c in site.counties.split(';')]
    if gps_county and gps_county not in documented:
        route_to_review(site, flag='county_mismatch',
            detail=f'GPS places entity in {gps_county}; counties field = {site.counties}')
        logger.warning(f'County mismatch: {site.site_id} counties={site.counties!r}'
                       f' but GPS -> {gps_county}')
```

**On mismatch:**
- Route entity to `manual_review_queue` with `flag = "county_mismatch"` and detail string
- Entity **continues through the pipeline** (not held or rejected) — proceeds to TSV output
  with its current `counties` value unchanged
- The manual review queue entry flags the discrepancy for human resolution

**Manual review options for county_mismatch:**
1. **Confirm GPS county** — update `counties` to GPS-derived county. If the entity ID prefix
   conflicts (e.g., `FR-` prefix on a confirmed Delaware County entity), correct the ID with
   a cascading update across all relationship tables (`access_point_parents`, `site_parent`,
   `trail_parents`, provenance tables) in a single transaction. This correction is permitted
   **before the cross-county run completes**. After the cross-county run merges the entity
   into the broader graph, treat the ID as immutable.
2. **Confirm documented county** — GPS is wrong or imprecise; close the flag, keep `counties`
   as-is, add explanation to `identity_notes`.
3. **Multi-county** — entity genuinely spans county lines; add GPS county to `counties` if
   missing, close the flag.

**Do not run this check when:**
- `gps_confidence` is LOW or NONE — not reliable enough to challenge the documented county
- GPS is null — §5.17a handles that case
...
**Step 5 — Unmapped token logging (IMP-116)**: For every token dropped in Step 4 (no vocabulary match, not already eliminated by Steps 1–3), write a provenance record:

```python
normalization_provenance.append({
    "entity_id": site["site_id"],
    "field": "features",
    "action": "unmapped_token_dropped",
    "raw_value": token,
    "reason": "no_vocabulary_match"
})
```

This step is non-blocking — it does not halt normalization or trigger a REVIEW flag. Its
purpose is to accumulate a vocabulary expansion candidates list that is surfaced during Stage 5
TSV Integrity Check. Reviewers use this list to identify recurring synonyms (e.g., `"Shelter
House"` → `"Picnic Shelter"`, `"Launch Ramp"` → `"Boat Ramp"`) that should be added to the
FEATURE_MAP reference (`utilities/na_feature_mapper_reference.md`) before the next county run.

Do not log tokens that were already eliminated by Steps 1–3 (activity terms, operational
content, named entities). Only log tokens that passed Steps 1–3 and failed vocabulary mapping.
...
- **GPS County Check** (§5.17b, IMP-067): WARNING + manual_review_queue flag="county_mismatch" if HIGH/MED GPS places entity outside documented counties; entity continues to output
...
------------------------------------------------------------
# END OF SITE NORMALIZATION CONTRACT v5.11