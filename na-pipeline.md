...
**Held entity exclusion (IMP-113):** Held entities must not appear in any entity TSV file. They are routed exclusively to the `held_entities` table. Before emitting any TSV row, verify the entity's ID is not in the held list. If a held entity ID appears in an entity TSV column (e.g., as a `parent_site_id` reference), that is a pipeline error — flag it, do not silently include it.

Also surface any unmapped token provenance records from Normalization §5.18 Step 5 as a **Vocabulary Expansion Candidates** section in the Stage 5 integrity check output. These are informational — they do not block output.
...
- **AP-to-Site reclassification check (IMP-114):** Review any Access Points that have `acres_raw` populated, a `description_raw` present, and governance distinct from the parent trail. These are candidates for reclassification as Sites before upsert. See AP Normalization Contract §10a for qualifying criteria, required steps, and prohibitions.
...
### Pre-Upsert MC Entity County Format Scan (IMP-115)

Before executing the upsert script, query the DB for any pre-existing MC entity whose `counties` field contains a semicolon followed by a space (`"; "`). This formatting error (e.g., `"Ottawa; Wood"` instead of `"Ottawa;Wood"`) is introduced when county values are concatenated without stripping whitespace, and is only discovered post-upsert without this scan.
...
-- repeat for trail_segments, trail_networks, access_points
```

Any matches must be corrected (strip spaces around semicolons) before the upsert run. The upsert's `ON CONFLICT DO UPDATE` will overwrite the bad value if the incoming record has the correct format — but the scan makes the discrepancy visible and auditable rather than silently auto-corrected.
...
Bash heredocs pass the entire file content as part of the command string. That string has
a silent size limit — content beyond the limit is truncated without any error or warning.
The result is a syntactically broken file that appears to have been written successfully.
This was the root cause of repeated truncation defects in `na_run_county.py` and
`na_pipeline_core.py`.
