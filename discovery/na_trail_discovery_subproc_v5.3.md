...
# TRAIL DISCOVERY SUB-PROCEDURE v5.3
...
This document supersedes all v5.2 and earlier Trail discovery logic.

------------------------------------------------------------
# CHANGES FROM v5.2 → v5.3

- **IMP-021 — Explicit trail use/surface fields**: `trail_use_type_raw` and
  `trail_surface_type_raw` upgraded from thin optional fields to Preferred fields
  with explicit capture guidance, anti-pattern examples, and a note that embedding
  these values inside `accessibility_raw` as a combined string is a normalization
  fallback — not a substitute for explicit fields. §7.1 updated accordingly.

------------------------------------------------------------
# CHANGES FROM v5.1 → v5.2

- **Added §7.1a Generic Trail Name Qualification (IMP-010)**: When a trail
  carries a generic name (Overlook Trail, Lake Trail, Nature Trail, Loop Trail,
  etc.) and is discovered within a multi-park system, the name_raw value must
  be qualified as `"[Park Name] — [Trail Name]"`. This prevents Resolution from
  falsely matching distinct trails that share common names across different parks.
  The original source name is preserved in alternate_names_raw. Qualification
  criteria and examples are defined in §7.1a.
- **Updated §6.2** with a generic name check step.
- **Updated §13 Quality Checklist** with generic name qualification check.
...
The Trail Discovery Sub-Procedure v5.2 provides the authoritative workflow
for:
...
**Generic name check (see §7.1a):** If the trail's name is a generic term
(Overlook Trail, Lake Trail, Nature Trail, Loop Trail, etc.) and you are
discovering within a multi-park system, apply the qualification rule in §7.1a
before recording name_raw.
...
Many parks use generic trail names — Overlook Trail, Lake Trail, Nature Trail,
Loop Trail, River Trail, Meadow Trail, and similar. When the same generic name
appears at multiple parks in a multi-park system, each trail is a distinct
entity but Resolution's identity matching would score them very high and risk
false merges.

**Qualification is required when ALL of the following are true:**

1. The trail name is a generic term that commonly appears at multiple parks
   (see examples below).
2. You are discovering within a **multi-park system** — a park district,
   county park system, conservancy network, or any agency that manages more
   than one distinct park property.
3. The trail is explicitly associated with a specific named park or preserve
...
The park name used must be the authoritative name of the containing park or
preserve — exactly as it appears in the source (unnormalized).

**Examples of generic names requiring qualification when in a multi-park system:**
...
- River Trail → "Scioto Grove Metro Park — River Trail"
- Meadow Trail → "Pickerington Ponds Metro Park — Meadow Trail"
- Woodland Trail → "Sharon Woods Metro Park — Woodland Trail"
- Ridge Trail → "Chestnut Ridge Metro Park — Ridge Trail"

**Names that are NOT generic and do NOT require qualification:**

A trail name is NOT generic when it contains a distinctive proper noun,
geographic name, historical reference, or other unique identifier that would
not commonly repeat across parks:

- "Slippery Elm Trail" ✅ (distinctive proper noun)
- "Buckeye Trail" ✅ (distinctive proper noun)
- "Blacklick Valley Greenway" ✅ (geographic name)
- "Heritage Rail Trail" ✅ (distinctive modifier)
- "Overlook Trail" ❌ (generic — qualify if in multi-park system)

**When NOT to qualify:**

- Single-park agencies (a trail at the only park managed by an entity does
  not require qualification, even if the name is generic).
- Multi-park systems where the generic name provably occurs only once — but
  if you cannot confirm this, qualify to be safe.
- Cross-tier greenway trails with established regional identities (e.g.,
  "Olentangy Trail") that span multiple parks but are managed as a single
  named corridor — these are NOT multi-park-system trail names and should
  not be qualified.

**Recording the qualification:**

- `name_raw`: the qualified form, e.g., `"Prairie Oaks Metro Park — Nature Trail"`
- `alternate_names_raw`: the published name as found, e.g., `"Nature Trail"`
- `identity_notes_raw`: add `"Name qualified per §7.1a — generic name in
  multi-park system; original: [Trail Name as published]"`

**Examples of correctly qualified records:**

```yaml
name_raw: "Glacier Hills Metropark — Overlook Trail"
alternate_names_raw: "Overlook Trail"
identity_notes_raw: "Name qualified per §7.1a — generic name in multi-park system; original: Overlook Trail"
```

```yaml
name_raw: "Chestnut Ridge Metro Park — Lake Trail"
alternate_names_raw: "Lake Trail"
identity_notes_raw: "Name qualified per §7.1a — generic name in multi-park system; original: Lake Trail"
```
...
recorded here:
- "Nature Trail" (if name_raw was qualified to "Prairie Oaks Metro Park — Nature Trail")
...
Capture this field explicitly whenever the source states or implies a use type.
Do not embed use type information inside `accessibility_raw` as a combined string.
`accessibility_raw` is for ADA/accessibility statements only; use type belongs here.

**Examples of source terms to capture:**
- "multi-use", "bike trail", "hiking trail", "equestrian trail",
  "water trail", "mountain bike trail", "nature trail", "paved multi-use path"

**Anti-pattern (IMP-021):**
- ❌ `accessibility_raw: "Paved multi-use (hiking, biking)"` — this conflates
  surface, use, and accessibility. Extract instead:
  - `trail_use_type_raw: "hiking, biking"`
  - `trail_surface_type_raw: "paved"`
  - `accessibility_raw: [leave blank unless source states ADA compliance or specific accessibility note]`

If the source genuinely provides no use type information, leave blank. Never infer.

---

### `trail_surface_type_raw` (PREFERRED — IMP-021)
Record exactly as found. Capture this field explicitly whenever the source
states a surface type — do not embed it inside `accessibility_raw`.

**Examples:**
- "asphalt" → record "asphalt"
- "gravel and dirt" → record "gravel and dirt"
- "crushed limestone" → record "crushed limestone"
- "natural surface" → record "natural surface"

If the source provides no surface information, leave blank. Never infer from
trail photos, maps, or category.
...
- Generic name qualification notices (§7.1a):
  "Name qualified per §7.1a — generic name in multi-park system; original: Overlook Trail"
- Cross-tier trail flags (Discovery Protocol v5.5 §18.3):
  "Cross-tier trail — primary manager may be [tier name/entity name]"
...
- ❌ Don't add type/description metadata to maps_raw entries —
  URLs only
- ❌ Don't leave a generic trail name unqualified when in a multi-park
  system — apply §7.1a
...
found within the district's park portfolio.
...
**Generic name note:** Conservancies managing multiple distinct preserves
are multi-park systems for purposes of §7.1a.
...
- identity_notes_raw with any trail/segment boundary questions,
  network membership notes, alternate name conflicts, and §7.1a
...
- ✅ name_raw recorded exactly as found (or qualified per §7.1a if generic)
- ✅ If §7.1a qualification applied: alternate_names_raw contains original
  published name; identity_notes_raw contains qualification notice
...
------------------------------------------------------------
# END OF TRAIL DISCOVERY SUB-PROCEDURE v5.2
