...
**How to save**: Use Python `urllib` via Bash (wget and curl are not available in this environment):
```bash
python -c "import urllib.request; urllib.request.urlretrieve('{URL}', 'County_Spreadsheets/{County}/source_files/{filename}')"
```

Or for multiple files in one script:
```python
import urllib.request, pathlib
src = pathlib.Path("County_Spreadsheets/{County}/source_files")
...
for url, name in files:
    urllib.request.urlretrieve(url, src / name)
    print(f"  Saved: {name}")
```
