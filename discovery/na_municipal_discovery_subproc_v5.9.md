...
# MUNICIPAL LANDS DISCOVERY SUB-PROCEDURE v5.12
...
This module supersedes Municipal Lands Discovery Sub-Procedure v5.11.
...
------------------------------------------------------------
# CHANGES FROM v5.11 → v5.12

- **IMP-099 — Municipal Cemeteries and Municipal Golf Courses**: Added mandatory cemetery
  and golf course search queries to §4.2 Step 2. Cemeteries may be managed under Public
  Works, City Services, or a standalone Cemetery Board rather than Parks & Recreation —
  discovery must check all municipal departments. Includes both targeted searches and
  Find A Grave / county auditor parcel layer verification.

------------------------------------------------------------
# CHANGES FROM v5.10 → v5.11

- **IMP-032 — Description Quality Gate**: Added cross-reference to IMP-032 in §4.3
  First-Pass Capture, `description_raw` section. Source descriptions that only restate
  the park name, location, governance, category, or acreage have zero information value
  and must be left blank. Full quality gate and bad/good examples table in
  `na_site_discovery_subproc.md` §7.3.

------------------------------------------------------------
# CHANGES FROM v5.9 → v5.10

- **IMP-029 — Municipality list pre-write to handoff (§4.1)**: The complete
  municipality list (names, types, official URLs) must now be written to the
  handoff document's Pre-Discovery Checklist section as the first action of
  Tier 6 — before any individual municipality searches begin. This creates a
  durable enumeration baseline that survives context breaks; the next session
  resumes from the checklist rather than reconstructing the list from scratch.

- **IMP-030 — Source table capture to handoff (§4.3)**: When a source page
  contains a structured parks table (e.g., a city Adopt-a-Park page listing
  all parks with names and addresses), that table must be written verbatim to
  the handoff document's Captured Source Data section immediately — in addition
  to staging YAML records. Chat history and context do not survive session
  boundaries; the handoff does. Without this step, a re-fetch is required at
  staging time whenever a session ends between discovery and staging.

- **IMP-031 — GPS capture during map verification (§4.4)**: When clicking
  into a Google Maps entity detail card to confirm an entity's identity, the
  GPS coordinates must be captured from the page URL (format: @lat,lon,zoom)
  and written to the handoff's Captured Source Data table. This converts the
  map verification pass into a combined verification + GPS pass, reducing or
  eliminating the need for a dedicated GPS return pass later. GPS captured this
  way is recorded as approximate pending GPS acquisition pass confirmation.

------------------------------------------------------------
# CHANGES FROM v5.8 → v5.9

- **IMP-001 — Large municipality batching**: Added §5.13 Large Municipality Batching
  Protocol. Municipalities with more than 100 parks require a mandatory pre-discovery
  enumeration step followed by alphabetical batching at 100 parks per batch. Covers
  batch labeling, session log requirements, staging file conventions, and the
  multi-batch completion criterion.
...
**Cemeteries and golf courses — additional mandatory searches (IMP-099)**: Municipal cemeteries and municipal golf courses are frequently not listed on parks pages. Run these searches separately for every municipality:
...
Also check the municipal website navigation directly for a "Cemetery" or "Golf Course" section — these are often in a separate department from Parks & Recreation (e.g., under Public Works or City Services). Municipal cemeteries → `category: Cemetery`, subtype "Public Cemetery" unless name evidence indicates otherwise. Municipal golf courses → `category: Recreation Facility`, `subtype: Golf Course`.
...
- **DESCRIPTION QUALITY GATE (IMP-032)**: Before staging, apply the stripping test — remove the park name, location, governance, category, and acreage from the text. If nothing substantive remains, the source description has zero information value; leave `description_raw` blank. "X Park is a neighborhood park in [City], Ohio." is a blank. See `na_site_discovery_subproc.md` §7.3 for the full quality gate and examples table.
...
**Why this matters**: The staging YAML is the output record, but the handoff is the
inter-session record. Chat context does not survive session boundaries. If the session
ends between discovery and staging (the most common break point), all address data
from the source page is lost and must be re-fetched. Writing the table to the handoff
eliminates that re-fetch entirely.

**What to capture in the handoff Captured Source Data section**:
- Source URL and fetch date
- The full table: park name, address (as listed), any GPS or acreage if present
- Leave a GPS column blank if not provided — to be filled at map verification or GPS pass
...
entity detail card to confirm an entity's identity (verifying address, phone number,
review count, or park name), **capture the GPS coordinates from the page URL at
the same time**. Do not treat verification and GPS capture as two separate passes.

- The URL format `@LAT,LON,ZOOMz` in the Google Maps address bar contains decimal
  coordinates. Extract lat and lon directly.
- Write these coordinates to the **Captured Source Data table in the handoff** for
  that municipality, in the GPS Lat / GPS Lon columns.
- Also write to `gps_lat_raw` / `gps_lon_raw` in the staging YAML record if it has
  already been written; if the YAML record hasn't been staged yet, the handoff table
  preserves the coordinates until staging.
- Annotate in `identity_notes_raw`: `"GPS from Google Maps detail card during map
  verification — treat as approximate until GPS acquisition pass confirmation."`

Applying this sub-step consistently converts the map verification pass into a
combined verification + GPS pass. For most Tier 6 entities, this will eliminate
the need for a dedicated return GPS pass, since the detail card is opened anyway
to confirm identity.
...
## 5.13 Large Municipality Batching Protocol ✨ NEW IN v5.9 (IMP-001)

### 5.13.1 Trigger

A municipality requires batched discovery when it has **more than 100 parks**. This
threshold applies to all entity types combined (Sites, Trails, Trail Segments, Access
Points) — not Sites alone. For known large cities (Columbus, Cleveland, Cincinnati,
Toledo, Akron, Dayton), assume batching is required before beginning. For others,
the full enumeration step (§5.13.3) will reveal the count.

Municipalities with 100 or fewer parks: proceed with standard single-session discovery.
...
the same source that governs discovery. Do not re-sort by `name_raw`; use the
source listing's display name to assign alphabetical position.

### 5.13.3 Mandatory Pre-Discovery Enumeration

Before any detail-page fetching begins, enumerate the full parks list:
...
**The enumeration list is the authoritative batch assignment.** Do not reassign a
park to a different batch mid-discovery because its alphabetical position seemed
different than expected.

### 5.13.4 Batch Labeling Convention

Each batch is labeled in session logs and discovery metadata as:
...
Columbus (CRP) — Batch 3 (Linden Park → Northmoor Park)
```

Use the same label consistently across the session log, staging YAML metadata, and
any handoff document references.

### 5.13.5 Staging File Conventions

All batches for a single municipality write to the **same staging YAML file**. Each
entity record must have a `---` separator (per Discovery Protocol §22.2). Batch
transitions do not require any special delimiter beyond the standard per-record `---`.

In discovery metadata (at the top of each batch's session log entry), record:
...
Maintain a running session log entry for each batch that records:
...
- Baseline seeds confirmed in this batch
- Batch status: `complete` or `in_progress`

A batch is `complete` only when all parks in it have been fetched or explicitly
accounted for (with a documented reason for any skip).

### 5.13.7 Baseline Tracking Across Batches

Baseline seeds for the municipality are tracked across all batches collectively.
As each batch completes, mark the seeds that were confirmed in that batch. After
all batches are complete, any unconfirmed seeds are flagged for review — the same
process as for a single-session municipality.

Do not flag seeds as unconfirmed until all batches for the municipality are done.
A seed alphabetically ahead of the current batch is not yet missing — it is pending.

### 5.13.8 Map Verification

Map verification for a large municipality runs as a single pass after **all batches
are complete** — not after each batch. This is an application of the IMP-015 rule:
complete all web discovery before any map verification. For a large municipality,
"all web discovery" means all batches complete.
...
Do not mark the municipality complete after the final batch detail-fetch. Map
verification and seed reconciliation must still follow.
