...
# SITE DISCOVERY SUB-PROCEDURE v5.10
...
This document supersedes all v4.x, v5.3, v5.4, v5.5, v5.6, v5.7, v5.8, and v5.9 Site discovery logic.

------------------------------------------------------------
# CHANGES FROM v5.9 → v5.10

- **IMP-002 — Edge venue scope clarification**: Added §3.1 explicitly listing recreational
  and civic venue types that are in scope even when they sit at the edge of what might
  intuitively be considered a "natural area." Golf courses, sports complexes, cemeteries,
  fairgrounds, gun ranges, and similar venues are in scope when publicly accessible and
  identity-bearing. Discoverers must not skip these because the site type seems marginal.
  Added golf courses and sports complexes to the §6.1 Step 1 search list.

- **IMP-073 — Private entertainment venue inclusion (Tier 8)**: Added §5c. Private
  recreational venues (golf courses, sports complexes, driving ranges, entertainment
  facilities with outdoor land use) are discoverable at Tier 8. No PRIVATE_VENUE_REVIEW
  flag is required; collect them as standard raw discovery records with `discovery_tier: 8`.
  Removed any prior guidance suggesting these should be flagged for review rather than
  collected.

------------------------------------------------------------
# CHANGES FROM v5.8 → v5.9

- **IMP-068 — Cultural institution category assignment rule**: Added §5b. Discovery sources
  (municipal parks websites, county GIS layers) frequently list cultural institutions (botanical
  gardens, conservatories, museums, science centers) alongside parks and recreation facilities,
  leading to erroneous Recreation Facility category assignments. Discoverers must now check the
  site name against the cultural institution pattern table in §5b before assigning category_raw.
  Matching names must receive the correct category; defaulting to Recreation Facility for these
  names is a data integrity error.
...
The following venue types are **in scope** for Site discovery even though they may sit at
the edge of what intuitively seems like a "natural area." Do not skip these because the site
type appears marginal or recreational rather than conservation-oriented:

- **Golf courses** (ALL — public, semi-private, and private member clubs) — category: Recreation Facility / subtype: Golf Course (IMP-099)
- **Sports complexes** — multi-field athletic parks, soccer complexes, tournament facilities
- **Cemeteries** (ALL — public, private, family, church, veterans, historic, active) — category: Cemetery; subtypes assigned per Site Vocabulary §7.4 inference rules. Do NOT assign Historic Site or Cultural Facility — Cemetery is its own top-level category (IMP-099)
...
- **Entertainment venues with outdoor land use** — outdoor amphitheaters, event parks, etc.

The inclusion criterion is **publicly accessible, named, identity-bearing land use** — not
whether the site fits a conservation or parks-department mold. When uncertain, collect it.
Normalization will assign the correct category; discovery should not pre-filter edge cases out.
...
Discovery sources — municipal parks websites, county GIS parks layers, municipal recreation
departments — frequently list cultural institutions alongside parks and recreation facilities.
This causes discoverers to default `category_raw` to a parks-adjacent value ("Recreation Facility",
"Park", or blank) even when the site name unambiguously identifies a museum, botanical garden,
or similar cultural institution.

**Rule**: Before assigning or leaving blank `category_raw`, check the site name (case-insensitive)
against the pattern table below. If a match is found, the listed category **must** be assigned.
Defaulting to "Recreation Facility" or leaving `category_raw` blank for a matching name is a
discovery error.
...
| "Conservatory" (standalone, i.e., not "X Conservatory at Y Park") | "Curated Biological Site" | "Botanical Garden" | If name is "[Name] Conservatory" as a primary identity → Curated Biological Site. Do not apply to "performing arts conservatory" contexts — those → "Cultural Facility". |
...
| "Museum" (as primary identity, e.g., "Franklin County History Museum") | "Museum" | assign from type context | Do not apply to "Museum Building" as a feature name within a historic site |
| "Science Center" / "Science Museum" | "Museum" | "Science Museum" | |
| "Hall of Fame" | "Museum" | record subtype by context | |

**Ambiguous cases**: If the name matches a pattern above but the site appears on a parks or
recreation website with no additional context, assign the category per the table and record in
`identity_notes_raw`: `"Category assigned per IMP-068 cultural institution rule; verify against
authoritative source."` Do not leave `category_raw` blank for a matching name.

**This rule takes precedence over source context**. A museum listed in a parks department
inventory is still a Museum; a botanical garden managed by a parks department is still a
Curated Biological Site.

------------------------------------------------------------
# 5c. PRIVATE ENTERTAINMENT VENUE INCLUSION — TIER 8 (IMP-073)

At Tier 8 (Private), private recreational and entertainment venues with publicly accessible
outdoor land use are **in scope** and must be collected as standard raw discovery records.

**In scope — collect without flag:**
- Public and semi-public golf courses (municipal-style pay-to-play; private member clubs
  are borderline — collect with identity note)
...
- Commercial outdoor recreation venues (zip lines, ropes courses, shooting sports facilities)
- Privately operated campgrounds open to the public

**Field mapping for Tier 8 private venues:**
- `ownership_raw`: "Private" (or the owning company/organization name)
- `governance_raw`: owning organization name
- `discovery_tier`: 8
- `category_raw`: assign appropriate raw category as found (e.g., "Golf Course", "Sports Complex",
  "Campground"); normalization will assign the controlled vocabulary value

**No PRIVATE_VENUE_REVIEW flag**: Do not add `PRIVATE_VENUE_REVIEW` or any equivalent hold
flag to Tier 8 private venue records. These are standard discovery records. If a venue is
truly ambiguous (e.g., invite-only private club with no public access), record the ambiguity
in `identity_notes_raw` and collect it — do not suppress it.

**Exclusions** — do not collect:
- Indoor-only venues (bowling alleys, gyms, indoor sports arenas) with no outdoor land component
- Retail commercial properties (sports equipment stores, etc.)
- Venues that are exclusively ticketed events with no persistent outdoor land identity
...
- Golf courses, sports complexes, fairgrounds, shooting ranges (see §3.1 — edge venues in scope)
...
------------------------------------------------------------
# END OF SITE DISCOVERY SUB-PROCEDURE v5.10