...
`township` and `municipality` are populated via spatial join against Census TIGER boundaries after GPS coordinates are confirmed. Use Claude in Chrome to query the Census TIGER REST API:

```
Base: https://tigerweb.geo.census.gov/arcgis/rest/services/TIGERweb/Places_CouSub_ConCity_SubMCD/MapServer
Layer 1: County Subdivisions (townships)
Layer 4: Incorporated Places (municipalities)
Query: STATE='39' AND COUNTY='027'  (adjust FIPS codes per county)
```

**GPS Error Detection**: If a site's GPS coordinates fall outside the expected county boundary in the TIGER spatial join (no township match), this indicates a GPS error — not a boundary issue. Resolution:
1. Flag the site as `GPS_ERROR` in Notes
2. Derive township from address or authoritative text source instead
3. Do not leave township blank simply because GPS failed the spatial join

**Multi-county sites**: Leave `township` blank. Document in Notes.

**Municipality layer quirk**: The Incorporated Places layer does not have a COUNTY field. Filter by spatial intersection with the county's township union polygon, or by known municipal GEOID values, to avoid pulling same-named places from other counties.
