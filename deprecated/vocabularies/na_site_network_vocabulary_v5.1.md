...
# SITE NETWORK VOCABULARY MODULE v5.1
...
- "heritage corridor" vs. "historic corridor" — check whether scope is
  cultural+natural (Heritage) or historic-only (Historic)
- "greenway network" — check whether primarily sites (Site Network) or
  trails (Trail Network)
- "inactive" vs. "dissolved" — check for formal dissolution documentation
- "proposed" vs. "under development" — check whether formal establishment
  has occurred
- County or municipal park system — check whether system-level branding
  exists or whether it is governance body only
...
- When uncertain about Site Network vs. governance body: include and flag
  with `SITE_NETWORK_UNCERTAIN`
...
4. **One value per field** — No multi-value types (Network Type, Status)
...
- Handle common variations (see Section 6)
...
- **Site Network Schema Module v5.x** (field definitions)
- **Site Network Discovery Sub-Procedure v5.x** (raw capture)
- **Resolution Engine v5.x** (conflict detection)
- **Normalization Engine v5.x** (vocabulary mapping)
- **Site Network Normalization Contract v5.x** (normalization rules)
- **TSV Output Specification v5.x** (output format)

------------------------------------------------------------
# END OF SITE NETWORK VOCABULARY MODULE v5.1
