...
------------------------------------------------------------
# 2. SITE NETWORK FIELDS (13 FIELDS, AUTHORITATIVE ORDER)
...
## 3.2 Network Type
- Must match a value from the Site Network Vocabulary Module v5.0.
...
- Blank if ownership is distributed among member sites or unclear.
...
- Supports bidirectional querying:
  - All Sites in a network: query member_site_ids
  - All networks for a Site: query site_network_members relationship table
...
## 3.15 Derived Label
- Computed, not stored.
- Formula (v5.0): **Network Type + " — " + Governance**
...
- It satisfies the identity rules in the Resolution Engine v5.0.
...
- Membership stored in `site_network_members` relationship table:
  ...
  - `site_id` (FK to sites)
- `member_site_ids` field in this schema is a convenience cache
  of site_id values from this table.
- Queryable both ways:
