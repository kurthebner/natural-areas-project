...
- /schemas
  - na_site_schema_v5.2.md
  - na_trail_schema_v5.2.md
  - na_trail_segment_schema_v5.1.md
  - na_access_point_schema_v5.1.md
  - na_site_network_schema_v5.2.md
  - na_trail_network_schema_v5.1.md
...
- /discovery
  ...
  - na_conservancy_discovery_subproc_v5.3.md
...
- na_conservancy_discovery_subproc_v5.3.md   (Tier 7)
...
------------------------------------------------------------
# 14. UTILITIES (NOT COUNTED AS MODULES)

The /utilities directory contains shared Python runtime code for the
pipeline. These files are not project documentation modules — they are
executable code that implements the pipeline stages.

**Parameterized pipeline model (v1.0, 2026-05-02):**
Each county run is defined by a JSON config file stored in its own
County_Spreadsheets directory:
  County_Spreadsheets/{County Name}/{county}_{state}_pipeline_config.json

The config holds all county-specific data: entity lists (normalized,
Stage 2 output), GPS queries, fallback coordinates, bounding box,
and run metadata. Claude writes the JSON; the driver runs it.

To execute a county pipeline:
  python utilities/na_run_county.py --county-dir "County_Spreadsheets/{County Name}"
  python utilities/na_run_county.py --config ".../{cou