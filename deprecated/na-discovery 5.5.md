...
1. Confirm all baseline seeds are confirmed or flagged as unresolvable
2. Confirm all 8 tiers have documented results — entities or null evidence block per entity type
3. Confirm the staging YAML is complete and readable (run a quick `yaml.safe_load` sanity check)
4. Update the handoff document — mark all completed tiers, list any held/unresolved entities,
   record open questions, and set Status to `DISCOVERY COMPLETE — PIPELINE READY`

**Then invoke the pipeline skill:**

Discovery is complete. Load the `na-pipeline` skill and pass it the staging file path.
The pipeline skill takes over from here — it does not re-run discovery.

Inputs the pipeline skill expects:
- `{county}_{state}_raw_discovery.yaml` — the staging file
- `{county}_{state}_session_log.md` — for session log updates at each stage
- `natural_areas_v5.db` (at `NASqlite/natural_areas_v5.db`) — the entity graph database

**Before invoking the pipeline skill, read `processing/na_processing_orchestration.md`** — it defines the full post-discovery pipeline sequence, stage owners, and failure paths. Do not hand off to the pipeline skill without reading it first.
