...
# DISCOVERY PROTOCOL MODULE v5.9
...
- Map and asset file preservation requirements
- Integration points with Resolution, Normalization, and Entity Upsert
- Provenance and audit requirements

This module supersedes Discovery Protocol Module v4.0, v5.2, v5.3, v5.4, v5.5, v5.6, v5.7, and v5.8.

------------------------------------------------------------
# CHANGES FROM v5.8 → v5.9

- **IMP-110 — Map and asset file preservation**: Added §24 Map and Asset File
  Preservation. Any PDF, GIF, JPG, SVG, or other file containing map data, trail
  maps, park brochures, or other reference material discovered during a county run
  must be downloaded and saved to the county's source_files/ subfolder at fetch time.
  File path is recorded in urls_raw and the handoff's Captured Source Data section.

------------------------------------------------------------
# CHANGES FROM v5.7 → v5.8

- **IMP-001 — Large municipality batching**: Added §23 Large Municipality Batching.
  Municipalities with more than 100 parks require alphabetical batching at 100 parks
  per batch, preceded by a mandatory full enumeration step. Full protocol in Municipal
  Discovery Subproc v5.9 §5.13.

------------------------------------------------------------
# CHANGES FROM v5.6 → v5.7

- **IMP-022 — Category field canonicalization**: Added §21 Category Field
  Standardization. `category_raw` is the single authorized field name for entity
  category. `park_type_raw` and `site_type_raw` are deprecated and must not appear
  in new staging records.
- **IMP-024 — YAML staging colon quoting**: Added §22.1 requiring any field value
  containing a colon to be enclosed in quotes. Unquoted colons are parsed as YAML
  key-value separators and cause pipeline failures.
- **IMP-025 — YAML staging record separators**: Added §22.2 requiring `---` document
  separators between every discovery record in a staging file. Missing separators
  cause block-end parse errors when multiple records are concatenated.
...
If an older session's staging YAML uses a deprecated field name, it must be renamed
to `category_raw` before the file is passed to the Resolution or Normalization stage.
The pre-processing script may handle this as a fallback, but the root cause is a
protocol violation and the staging record must be corrected at the source.
...
This applies to Sites, Trails, Trail Segments, Trail Networks, Site Networks, and
Access Points. All entity types use `category_raw` if they have a category field.

------------------------------------------------------------
# 22. YAML STAGING FORMAT REQUIREMENTS

All discovery staging files are YAML. The following formatting rules are mandatory
for all records in all staging files. Violations cause pipeline failures.

## 22.1 Colon Quoting (IMP-024)

Any field value that contains a literal colon character must be enclosed in
double quotes. In YAML, an unquoted colon followed by a space is interpreted as a
key-value separator, which breaks the parse.
...
This applies to all fields. When in doubt, quote the value. Quoted values that
don't need quoting are valid YAML; unquoted values with colons are not.

## 22.2 Record Separators (IMP-025)

Every discovery record in a staging file must be preceded by a `---` YAML document
separator. Omitting separators causes block-end parse errors when multiple records
are concatenated.
...
record, even when the file already ends with another record. Never rely on blank
lines or indentation alone to delimit records.
...
Municipalities with more than 100 parks require alphabetical batching. A single
discovery session cannot feasibly fetch and stage hundreds of park detail pages, and
the protocol's baseline tracking and map verification steps require a clear completion
state for the municipality as a whole.

**Trigger**: >100 parks total (all entity types) in the municipality.

**Method**: Alphabetical, 100 parks per batch. Batch boundaries are determined by a
mandatory pre-discovery full enumeration of the parks listing — you must know the
complete list before assigning batches.

**All batches write to the same staging YAML file.** Standard `---` separators apply.

**Map verification runs once, after all batches are complete** — not per-batch.

**Completion**: Municipality is complete when all batches are marked complete in the
session log, map verification is done, and all baseline seeds are confirmed or flagged.

Full protocol — batch labeling convention, session log requirements, staging metadata
format, baseline tracking across batches, and completion criteria — is in Municipal
Discovery Subproc v5.9 §5.13.

------------------------------------------------------------
# 24. MAP AND ASSET FILE PRESERVATION (IMP-110)

During discovery, authoritative source files that contain map data, trail layouts,
park brochures, wildlife area boundary maps, or other reference material must be
downloaded and saved to a persistent location in the county's folder — not merely
linked. Linked URLs can break between sessions or become unavailable; the saved file
is the durable record.

**Trigger**: Any PDF, GIF, JPG, SVG, PNG, or other binary file that is:
- A park or trail map
- A wildlife area boundary or hunting area map
- A park or site brochure containing layout, amenity, or access information
- A water trail guide or paddling brochure
- Any other file containing geographic or facility reference data for an entity
  being discovered

Informational text pages (HTML), agency homepages, and directory listings are NOT
covered by this rule — only binary asset files with map or facility content.

**Where to save**: `County_Spreadsheets/{CountyName}/source_files/`

Create the `source_files/` subdirectory if it does not already exist.

**Naming convention**: Use a descriptive name that identifies the entity and source:
```
{entity_slug}_{source_slug}.{ext}
```
Examples:
- `van_buren_sp_trail_map.pdf`
- `hancock_wa_1_boundary.pdf`
- `blanchard_river_water_trail_guide.pdf`
- `findlay_island_park_aerial.jpg`

When the file is a county-wide source (e.g., a hunting area map covering all county
WAs), use a county-level prefix:
- `hancock_county_hunting_area_maps.pdf`

**How to save**: Use the bash `wget` command targeting the county source_files path:
```bash
wget -q -O "/sessions/.../mnt/Natural Areas Project v5/County_Spreadsheets/{County}/source_files/{filename}" "{URL}"
```

**Record the save**: After saving:
1. Add the local file path to `urls_raw` of the relevant entity record alongside
   the original URL.
2. Note the saved filename in the handoff's Captured Source Data section for the
   entity or source table it belongs to.

**When to save**: At first fetch — when the file is originally retrieved during
discovery. Do not defer to a later session. A file noted as "should download" but
not yet saved is a protocol gap.

**Scope**: This rule applies to all eight discovery tiers. It is not limited to
ODNR maps or any single source type.

------------------------------------------------------------
# END OF DISCOVERY PROTOCOL MODULE v5.9
...
When the file is a county-wide source covering multiple entities (e.g., a single
hunting area map PDF for all county WAs), use a county-level prefix:
- `hancock_county_hunting_area_maps_index.pdf`

**How to save**: Use the bash wget command targeting the county source_files path:

    wget -q -O "{county_source_files_path}/{filename}" "{URL}"

**Record the save**: After saving:
1. Add `local:{filename}` to `urls_raw` of the relevant entity record alongside
...
**Retroactive application**: When resuming a session mid-county, download any maps
or asset files that were fetched in prior sessions but not yet saved. Check the
handoff's urls_raw entries and Captured Source Data section for PDF or image URLs
that lack a corresponding `local:` entry.
