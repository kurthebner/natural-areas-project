...
interpreted, preserved, and integrated into the v5.x Natural Areas pipeline.
...
- **identity_notes_raw recognized as hint field**: `notes_raw` renamed to
  `identity_notes_raw` across all entity sub-procedures; baseline hint
  field list updated accordingly
- **maps_raw clarified**: plain semicolon-delimited URL list only; type
  and description metadata not collected at any stage
- **Pipeline integration section**: stage numbers removed to avoid
  collision with the v5.x pipeline stage numbering in Processing
  Orchestration; descriptive step headers used instead
- **All cross-module references updated to v5.x**

------------------------------------------------------------
# CHANGES FROM v4.0 → v5.0

- `role` and `access_level` removed from baseline field hints — fields
  deleted from Access Point schema
- `features_raw`, `difficulty_raw`, `accessibility_raw`, `maps_raw`
  added as recognized hint fields
- `township_raw` and `municipality_raw` explicitly excluded from
  baseline interpretation — GIS-derived only
- Discovery = Collection principle reinforced — baseline rows are raw
  seeds, never normalized values
- All version references updated to v5.0

------------------------------------------------------------
# 1. PURPOSE

The County Baseline Module v5.1 defines:

- What a county baseline is
- How baseline spreadsheets are interpreted
- How baseline identity is preserved as **Tier-0 seeds**
- How baseline integrates with discovery and resolution in v5.x
...
- Full auditability
- Full compatibility with the v5.x tiered pipeline
...
All normalization and schema enforcement occur later in the v5.x
pipeline.

### Fields that must not appear in baselines:

- `township` — must never be supplied via baseline; GIS-derived only
- `municipality` — must never be supplied via baseline; GIS-derived
  only

If a baseline row contains township or municipality values, those
values must be:

- Preserved in raw metadata for provenance
- Never used to populate the township or municipality fields in the
  normalized record
...
- **Access Points** → must resolve to a single primary county or be
  flagged
...
Baseline defines an initial list of **candidate entities** for a
county: "This thing probably exists here."
...
Entity type is determined later by discovery, resolution, and
normalization.

## 7.4 Baseline does not determine parent/child relationships
Parent Site, Trail, Trail Segment, and network relationships are
assigned later.
...
These are GIS-derived fields and must never be populated from
baseline data.

------------------------------------------------------------
# 8. BASELINE INTEGRATION WITH THE v5.x PIPELINE

In v5.x, the baseline operates as **Tier-0** and runs **after** all
authoritative tiers. The following describes how baseline flows through
the pipeline; for full stage definitions see
`processing/na_processing_orchestration_v5.x.md`.
...
## GPS Acquisition
GPS Acquisition runs after Resolution Pass 1 for Site, Trail, and
Access Point entities — including those seeded from baseline. Baseline
GPS hints feed into GPS Acquisition as candidate coordinates but are
subject to the same acquisition and validation rules as all other
entities.

## Normalization
Baseline raw values flow into normalization as hints. Normalization
applies schema rules; baseline raw values are preserved in provenance.
...
Baseline is **never** allowed to override authoritative agency or GIS
data.
...
- `baseline_id`
- Any anomalies detected (including township/municipality value
  attempts)
...
- Discovery Protocol Module v5.x (as Tier-0 inputs)
- Resolution Engine v5.x
- Normalization Engine v5.x
- Audit & Logging Module v5.x

Baseline output is never used directly for normalized entities; it
always flows through Resolution and Normalization.
...
------------------------------------------------------------
# END OF COUNTY BASELINE MODULE v5.1
