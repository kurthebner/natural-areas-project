...
# CHILD SITE RULES MODULE v5.1
(Authoritative Ontological Rules for Parent–Child Site Relationships)

This module defines the **v5.1 ruleset** governing when a Site may legitimately function as a **child identity-bearing land unit** within another Site.
...
All Site fields are defined in the **Site Schema Module v5.0**.

------------------------------------------------------------
# CHANGES FROM v5.0

- Field name updated: `county_list` → `counties` throughout
- Module dependencies updated to reference v5.0 modules
- All rules remain substantively unchanged

**CHANGES FROM v5.0 (Clinton County session):**
- **OBS-008**: §10 Discovery Rules expanded — child site identification is a
  normalization-phase decision; discovery creates independent raw records and notes
  suspected parent relationships; `parent_site_id` is never assigned during discovery
...
------------------------------------------------------------
# 6. COUNTY INHERITANCE RULES (v5.0)

The following rules govern the relationship between parent and child `counties` values.

### 6.1 Default Rule — Subset Inheritance
A child Site's `counties` must be a **subset** of the parent Site's `counties`, unless authoritative sources explicitly document otherwise.
Identical sets are valid.

### 6.2 Permitted Variation — Documented Exceptions
If authoritative sources explicitly document that a child Site occupies a county not listed for the parent, the child Site's `counties` must reflect that documentation.
...
The child Site's `counties` must reflect **its actual documented footprint**, not the parent's.
...
7. Validate that the child's `counties` is a subset of the parent's unless explicitly documented otherwise.
8. Validate that State matches the parent unless explicitly documented otherwise.

------------------------------------------------------------
# 10. DISCOVERY RULES

**Child site identification is a normalization-phase decision, not a discovery-phase decision.**

During discovery, a discoverer may notice that a feature appears to be an internal
unit of a larger Site. This observation is valuable and must be preserved — but the
formal determination that something IS a child Site, and the assignment of
`parent_site_id`, happens in the normalization phase using this module.

## 10.1 During Discovery
- Create an **independent raw record** for every identity-bearing internal unit
- Do not assign `parent_site_id` in the raw record
- Do not suppress a potential child Site because "it's just part of [parent]"
- Note the suspected parent relationship in `notes_raw`:
  `"SUSPECTED CHILD SITE of [parent name] — parent_site_id to be assigned in normalization"`
- Evaluate the feature against this module's identity rules (§2) to decide whether
  to create a record at all — but if it passes, always create it as independent
- Flag ambiguous cases: `CHILD_SITE_REVIEW — identity and parent relationship require normalization determination`

## 10.2 What Discovery Must Not Do
- Assign `parent_site_id` during raw record creation ❌
- Suppress a feature because it appears to be internal to a larger Site ❌
- Infer parent relationships from layout, proximity, or assumption ❌
- Create child Sites based on features, amenities, or unnamed zones ❌
- Route features (playgrounds, shelters, parking lots) as child Sites ❌

## 10.3 What Discovery Must Do
- Evaluate internal names against §2 identity rules
- Create independent raw records for identity-bearing internal units
- Note suspected parent in `notes_raw`
- Flag ambiguous names for normalization review
- Reject undocumented names per §3 evidence requirements
...
- Ensure that `counties` follows the v5.0 subset rule
...
------------------------------------------------------------
# END OF CHILD SITE RULES MODULE v5.0
