...
- **OBS-005**: §4.15 added — NRHP archaeological sites on private land entity type rules
- **OBS-008**: §2.14 added — child site identification phase clarification (discovery vs.
  normalization); §8 updated to reinforce that child site decisions are normalization-phase only
...
## 2.14 Child Site Identification Is a Normalization-Phase Decision
During **discovery**, a discoverer may note that a feature appears to be an internal
unit of a larger Site (e.g., a campground within a state park, a named area within
a county park). This observation belongs in `notes_raw`.

The formal determination that something IS a child Site — and the assignment of
`parent_site_id` — happens in the **normalization phase**, using the Child Site
Rules Module v5.0.

Discovery must not:
- Assign `parent_site_id` during raw record creation
- Suppress a potential child Site because "it's just part of [parent]"
- Assume parent-child relationships without source confirmation

If a feature appears to be a child Site, create an independent raw record for it
and note the suspected parent relationship in `notes_raw`. Resolution and
normalization will confirm or reject the relationship.
...
## 4.15 NRHP Archaeological Sites on Private Land
National Register of Historic Places listings for archaeological sites, mounds,
and cultural features that sit on private land require careful entity-type decisions:

**If public visitor access is documented (formal easement, posted public access):**
- Entity type: Site
- Category: Archaeological Site or Historic Site
- Tier: 8 (Private) unless a government agency holds the easement (then appropriate tier)
- Create normally; record access conditions in `notes_raw`

**If no public access is documented but the feature is identity-bearing and mapped:**
- Entity type: Site
- Category: Archaeological Site or Historic Site  
- Tier: 8 (Private)
- Flag: `NO_PUBLIC_ACCESS — NRHP listed; access confirmation required`
- Do not suppress — NRHP listings are identity-bearing by definition

**If the NRHP feature is within an existing Tier-8 Site (e.g., a mound within a
private hunting preserve):**
- Entity type: child Site (Site with `parent_site_id`)
- Normalization phase assigns `parent_site_id`
- Discovery phase creates an independent raw record with suspected parent noted in `notes_raw`

**NRHP listings for private archaeological sites with no on-the-ground access** are
still valid records — the NRHP designation itself is the identity-bearing element.
These sites may become accessible in the future, and their record provides continuity.
...
**Child Site determination is a normalization-phase decision.** During discovery,
create independent raw records for all suspected child Sites. Do not suppress a
feature because it appears to be internal to a larger Site — create the record
and note the suspected parent relationship in `notes_raw`. The formal assignment of
`parent_site_id` is made by the Normalization Engine using the Child Site Rules
Module v5.0, not during discovery or resolution.
