...
- Added §11.8 Cross-Tier Trail Canonicalization (IMP-018):
  When the same named trail is staged at multiple discovery tiers (e.g., by both a metro
  park district at Tier 3 and a municipality at Tier 6), the engine detects the cross-tier
  duplicate cluster in Phase 3, merges the records in Phase 4, and assigns canonical status
  to the record from the primary managing entity's tier. The non-canonical record's data is
  fully preserved in resolution_provenance. Discovery Protocol v5.5 §18 and the relevant
  subprocs define how discovery records are flagged; this section defines how the engine
  acts on those flags.

------------------------------------------------------------
# CHANGES FROM v5.3 → v5.4

- Expanded §9.5 (Identity Matching field model) to reflect the full v5.x discovery field
  model, explicitly listing all raw fields used in matching and removing the deprecated
  gps_raw, maps_raw, map_url, and geometry_raw fields.
- Expanded §11.3 (Field Strategy Assignment) to include a complete field-strategy table
  for all six entity types, including Trail Segment, Trail Network, Site Network, and
  Access Point fields.
- Added §12.6 Final Resolved Entity Structure with explicit block definitions.

------------------------------------------------------------
# CHANGES FROM v5.0 → v5.3

- Updated all cross-module references to v5.x
- Aligned field model with Discovery Output Specification v5.x (gps_lat_raw, gps_lon_raw,
  urls_raw, location_raw, organizational cluster)
- Added parent_resolution merge strategy
- Added metadata_union merge strategy
- Expanded Phase 5 lineage preservation rules

------------------------------------------------------------
# 1. PURPOSE

The Resolution Engine v5.5 provides the authoritative, deterministic workflow for transforming **Raw Discovery Records v5.x** into **Resolved Entities v5.x**. It performs the mechanical execution of identity detection, similarity scoring, merge decisions, field‑level merging, parent resolution, and metadata propagation.
...
Resolution Engine v5.5 is responsible for:
...
- assigning canonical tier status to cross-tier trail records (§11.8)

Resolution Engine v5.5 is **purely mechanical**. It does not interpret, infer, normalize, or decide between conflicting values — except for the canonical tier assignment described in §11.8, which is a deterministic rule-driven operation, not an inference.

------------------------------------------------------------
# 2. SCOPE

Resolution Engine v5.5 governs:
...
- canonical tier assignment for cross-tier Trail records

Resolution Engine v5.5 does **not**:

- normalize vocabulary
- choose canonical values
- apply tier authority (except for the cross-tier trail rule in §11.8)
...
Resolution Engine v5.5 sits between:
...
7. **Assign** canonical tier status to cross-tier Trail records (§11.8)
8. **Produce** deterministic Resolved Records v5.x
...
### 4.2 Purity
Resolution Engine v5.5 must not modify, infer, or normalize raw values or metadata.
...
### 5.1 Inputs
Resolution Engine v5.5 consumes:
...
### 5.2 Outputs
Resolution Engine v5.5 produces:
...
- Canonical tier assignments for cross-tier Trail records
...
Resolution Engine v5.5 executes the following phases:
...
4. **Phase 4 — Field‑Level Merging**
   Merge raw values using deterministic strategies while preserving conflicts. Includes cross-tier trail canonicalization (§11.8).
...
Resolution Engine v5.5 depends on:
...
- **Discovery Protocol Module v5.5**
  (cross-tier trail flagging semantics — §18)
...
Resolution Engine v5.5 must not duplicate logic from these modules.
...
Resolution Engine v5.5 does not contain any identity logic. Instead, it must call the following rule sets from the Resolution Rules Module v5.x:
...
Phase 4 transforms each merge cluster from Phase 3 into a single **Resolved Entity v5.x**. This phase merges raw values, preserves conflicts, assigns canonical tier status to cross-tier Trail records (§11.8), and assembles metadata blocks. It does not normalize, infer, or choose canonical values outside of the canonical tier assignment rule. All raw values and all metadata must be preserved exactly as discovered.

Field‑level merging is mechanical and rule‑driven. The engine applies merge strategies to each field, using the updated v5.x discovery field model and the merge semantics defined for v5.5.
...
- normalize values
- infer missing values
- choose between conflicting values (except via the canonical tier rule in §11.8)
- modify raw metadata
...
------------------------------------------------------------
# 11.3 FIELD STRATEGY ASSIGNMENT (v5.5)
...
- merge provenance
- canonical tier assignment (for Trail clusters with cross-tier records — see §11.8)

Parent IDs are not yet resolved.
That occurs in Phase 5.

------------------------------------------------------------
# 11.8 CROSS-TIER TRAIL CANONICALIZATION (IMP-018)

When the same named Trail is staged at multiple discovery tiers — for example, by a metro park district at Tier 3 and a municipality at Tier 6 — the ordinary merge process will produce a cluster containing records from different tiers. This section defines how the engine assigns canonical tier status within that cluster.

### 11.8.1 Detection Criteria

A merge cluster is a **cross-tier Trail cluster** when all of the following are true:

- `entity_type` = Trail for all records in the cluster
...
- at least one record carries an `identity_notes_raw` entry containing the substring
  `"Cross-tier trail"` (as required by Discovery Protocol v5.5 §18.3)

The engine must not apply this section to clusters that do not meet all three criteria.

### 11.8.2 Canonical Record Selection

Within a cross-tier Trail cluster, the engine assigns canonical status to the record from the **primary managing entity's tier**. This is determined as follows, in order:

1. **Governance field evidence** — If exactly one record in the cluster has `governance_raw` that unambiguously identifies it as the primary manager (e.g., "Metro Parks manages and maintains [trail name]", "City of X is responsible for all maintenance"), that record's tier is canonical.

2. **Absence of cross-tier flag** — If exactly one record in the cluster does NOT carry the `"Cross-tier trail"` flag in `identity_notes_raw`, that record's tier is canonical. (Discovery Protocol v5.5 §18.3 requires the flag only on non-canonical records.)

3. **Tier precedence fallback** — If the above criteria are inconclusive, the engine applies tier precedence: lower tier number = higher authority. The record with the lowest discovery tier number becomes canonical. A review set is also emitted, as described in §11.8.4.
...
- `selection_basis`: which criterion from §11.8.2 was used ("governance_evidence", "flag_absence", or "tier_precedence_fallback")
- `governance_raw_values`: all distinct `governance_raw` values from the cluster, for audit

### 11.8.4 Non-Canonical Record Handling

The non-canonical record(s) in a cross-tier Trail cluster are **not discarded**. Their data must be:

- fully merged using the standard field strategies in §11.3
- preserved in `resolution_provenance` with `role: non_canonical`
- flagged so that the Normalization Engine suppresses them from TSV output and DB upsert

The Normalization Engine v5.x is responsible for applying the suppression flag. The Resolution Engine only sets it.

### 11.8.5 Review Set Emission

In addition to normal merge processing, the engine must emit a review set for the cross-tier Trail cluster when:

- the tier_precedence_fallback criterion was used (§11.8.2 rule 3)
...
### 11.8.6 Scope and Limits

This section applies **only** to Trail entities. Sites, Trail Segments, Trail Networks, Site Networks, and Access Points are not subject to canonical tier assignment in this version.

Cross-tier duplication for non-Trail entity types is handled by the standard review set mechanism (§10.6). Future versions of the Resolution Engine may extend canonical tier assignment to other entity types.
...
------------------------------------------------------------
# 12.6 FINAL RESOLVED ENTITY STRUCTURE (v5.5)
...
- canonical tier assignment (for cross-tier Trail clusters — see §11.8.3)
