...
- Field counts updated for all six entity types to reflect v5.0 schema changes:
...
- How results integrate with the Audit & Logging Module v5.0
...
- Full compatibility with the v5.0 ontology
...
v5.0 TSV Output Specifications.

## 6.1 Site (26 fields)
- Site Name (identity anchor) → Field 1
- County → Field 3
- Township → Field 4 (GIS-derived)
- Municipality → Field 5 (GIS-derived)
- GPS Lat → Field 8
- GPS Lon → Field 9
- Features → Field 13
- Parent Site → Field 25
- Derived Label → Field 26
...
All failures must be logged in the Audit & Logging Module v5.0.
...
- Normalization Engine v5.0 (pipeline halt signal)
...
------------------------------------------------------------
# END OF TSV INTEGRITY CHECK MODULE v5.0
