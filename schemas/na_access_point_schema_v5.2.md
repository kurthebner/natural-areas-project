...
# ACCESS POINT SCHEMA MODULE v5.2
...
- **IMP-126** — Corrected stale `integer` type annotation on Access Point ID (§3.17)
  to `TEXT` with explicit `OH-{COUNTY}-{TYPE}-{SEQ}` format note. DB schema was already
...
- Must be a valid TEXT ID in OH-{COUNTY}-{TYPE}-{SEQ} format matching the entity's access_point_id.
