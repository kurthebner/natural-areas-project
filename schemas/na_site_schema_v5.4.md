...
- **IMP-127** — Corrected `counties` (§4.14) and `urls` (§4.23) type annotations from
  `array (JSON) / semicolon-delimited (TSV)` to `TEXT, semicolon-delimited`. SQLite has
  no native array type; the DB stores these fields as semicolon-delimited TEXT, identical
  to the TSV format. The "Array in JSON" half of the prior description did not reflect
  actual storage.
- **IMP-127** — Corrected `created_at` (§4.25) and `updated_at` (§4.26) type
  annotations from `timestamp` to `TEXT, ISO 8601 format (YYYY-MM-DDTHH:MM:SSZ, UTC)`.
  SQLite has no native TIMESTAMP type; TEXT is the correct SQLite storage class for
  datetime values.

------------------------------------------------------------
# CHANGES FROM v5.2 → v5.3

- **IMP-126** — Corrected stale `integer` type annotation on `site_id` (§4.1) and
  `parent_site_id` (§4.24) to `TEXT` with explicit `OH-{COUNTY}-{TYPE}-{SEQ}` format
  note. DB schema was already correct; this fixes documentation drift introduced before
  the IMP-107 global ID migration.
...
## 4.1 site_id
TEXT, required — format: OH-{COUNTY}-{TYPE}-{SEQ} (e.g., OH-OTT-S-001); assigned by Upsert Engine
...
## 4.14 counties
TEXT, semicolon-delimited, required, alphabetical (stored identically in DB and TSV)
...
## 4.23 urls
TEXT, semicolon-delimited, optional (stored identically in DB and TSV)

## 4.24 parent_site_id
TEXT, optional, FK to sites.site_id — must match OH-{COUNTY}-{TYPE}-{SEQ} format
