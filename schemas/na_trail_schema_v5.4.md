...
- **IMP-127** — Corrected `alternate_names` (§3.2) and `counties` (§3.7) type
...
- **IMP-126** — Corrected stale `integer` type annotation on Trail ID (§3.19) to
...
- **IMP-054 — §5.4 Parent Site relationship added**: Documents the `trail_parents`
  ...
  See also `na_trail_parent_site_schema.md` for the full schema specification.
...
- Must be a valid TEXT ID in OH-{COUNTY}-{TYPE}-{SEQ} format matching the entity's trail_id.
...
Full schema for the `trail_parents` table: see `na_trail_parent_site_schema.md`.
