...
# SITE NETWORK SCHEMA MODULE v5.4
...
- **IMP-127** — Corrected `counties` (§3.8) and `states_included` (§3.9) type
...
- **IMP-126** — Corrected stale `integer` type annotation on Network ID (§3.16) to
...
- **Governance Entity Role added to Purpose**: Site Networks now explicitly serve
  as the canonical record for any multi-site managing organization, making them
  the single storage location for org-level website URLs and avoiding duplication
  across individual Site and Trail records.
- **Org Type field added**: New controlled vocabulary field (position 3) classifying
  the organizational category of the governance entity responsible for or associated
  with the network. Vocabulary defined in Site Network Vocabulary Module v5.2.
- **Field count updated**: 16 fields (was 15).
...
**Governance Entity Role**: Site Network records serve as the canonical record
for the managing organization associated with each network. The `url` field on
a Site Network record is the authoritative location for that organization's
primary website — it is captured once here and not duplicated across individual
Site or Trail records. The `governance` field on member Site and Trail records
references the organization's name; the URL is retrieved by joining to the
Site Network record. This applies to all multi-site managing organizations:
municipal parks departments, county park districts, state agency regional
offices, land trusts, conservation authorities, and federal land managers.
For rare single-site operators, the `url` field on the individual Site record
remains the fallback.

This schema is authoritative for **Site Network structure**.

------------------------------------------------------------
# 2. SITE NETWORK FIELDS (16 FIELDS, AUTHORITATIVE ORDER)
...
- Distinct from Network Type: Network Type describes what the network is;
  Org Type describes what kind of organization governs or manages it.
- Optional. Leave blank if the governing organization does not clearly fit
  any vocabulary value, or if the network has no single governance entity.
- Must not be inferred from the network type or member site ownership alone.
...
- For networks whose Site Network record also serves as the canonical
  governance entity record, this field holds the organization's primary
  website URL.
...
- Site Network Vocabulary Module v5.2
...
------------------------------------------------------------
# END OF SITE NETWORK SCHEMA MODULE v5.2
